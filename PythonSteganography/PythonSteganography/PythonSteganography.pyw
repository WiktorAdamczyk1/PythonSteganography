...
class Application(Frame):
    ...
    def open_file(self):
        ...
        if left_img.mode == 'L':
            ...
            if self.available <= 0:
                self.available_label['text'] = 'Available Size: 0 Bytes'
            else:
                self.available_label['text'] = 'Available Size: ' + str(self.available) + 'Bytes'
    ...
    def decry(self):
        if self.file_name == '':
            ...
            clear_message = lsb.reveal(self.file_name)
            if self.msg_box.get('1.0', END) != '':
                self.msg_box.delete('1.0', END)
            self.msg_box.insert(END, 'Hidden message: "' + clear_message + '".')
    ...
    def encry(self):
        ...
        if self.file_name == '':
            ...
            secret = lsb.hide(self.file_name, hide_msg)
            secret.save(new_file_name)

            global right_img
            right_img = Image.open(new_file_name)
            w, h = right_img.size
            # resize img
            scale_w = img_display_width / w
            scale_h = img_display_height / h
    ...
    def test_image(self):
        if self.file_name == '':
            ...
            if clear_message == None:
                self.msg_box.insert(END, "There is no hidden message")
            else:
                self.msg_box.insert(END, "There is a hidden message")
    ...
    def create_widgets(self):
        ...
        self.mode_label = Label(show_frame, text='Available Size: ')
        ...
        decry_button.pack(side=RIGHT)

        encry_button = Button(en_de_cry_frame, text='Encryption', command=self.encry)
        encry_button.pack(side=LEFT)

        encry_button = Button(en_de_cry_frame, text='Test', command=self.test_image)
...
app = Application()
app.master.title('Steganography')
app.mainloop()
